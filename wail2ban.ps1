...
function helpx 
	...
	"wail2ban"
	"`n"
	"wail2ban is an attempt to recreate fail2ban for windows,"
	"hence [w]indows f[ail2ban]."
	" "
	"wail2ban takes configured events known to be audit failures, or similar,"
	"checks for IPs in the event message,"
	"and given sufficient failures, bans them for a small amount of time."
	" "
	"Settings: "
...
################################################################################
#  Constants
$CHECK_WINDOW = 120			# check the most recent X seconds of log
$CHECK_COUNT  = 5			# Ban after this many failures in search period
...
$logFile         = $wail2banInstall+"wail2ban.log"
$ConfigFile      = $wail2banInstall+"wail2ban.ini"
...
switch -regex -file $ConfigFile 
	...
	"^\s*([^#].+?)\s*=\s*(.*)" 
		...
		if ( $EventTypes -match $Header ) 
			...
			{ 
			switch ($Header) { "Whitelist" { $WhiteList += $Match1; } }	
			}
		}
...
function rule_exists ($IP) 
	...
	if ($rule)
		...
		if ($result -match "----------") { return "Yes"}  else { return "No" }
		}
	}
...
function whitelisted($IP) 
	...
	foreach ($white in $Whitelist) 
		...
		if ($white.contains("/")) 
			...
			if 
				(
				(([net.ipaddress]$IP).Address -Band ([net.ipaddress]$Mask).Address ) 
				-eq`
...
if ($args -match "-config") 
	...
	for ($i = 1; $i -lt 5; $i++) 
		{ write-host (""+[math]::pow(5,$i)+", ") -foregroundcolor "cyan" -nonewline } 
